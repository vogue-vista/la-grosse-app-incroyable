...
st.markdown("<div style='background-color: #1e1e24; padding: 12px; border-radius: 8px; border-left: 5px solid #66fcf1; margin-bottom: 20px;'><span style='font-size:13px; color:#c5c6c7;'>• 💰 Utilisateur <b>Alex_MTL</b> a généré <b>340 $</b> en 24h avec son shop !</span><br><span style='font-size:13px; color:#c5c6c7;'>• 🎯 L'IA de <b>Félix_SMMA</b> a extrait 150 leads de dentistes (Bouton 1).</span></div>", unsafe_allow_html=True)
...
if not st.session_state.compte_actif:
    ...
    with tab1:
        st.header("Extracteur & Rédacteur Commercial IA")
        metier = st.text_input("Métier ciblé :", "Plombier")
        ville = st.text_input("Ville :", "Montreal")
        if st.button("🔥 Déclencher la prospection"):
            if st.session_state.credits_restants > 0:
                st.session_state.credits_restants -= 1
                leads = outils.executer_scraping_reel(metier, ville)
                for lead in leads:
                    st.subheader(f"🎯 Entreprise : {lead['nom']}")
                    prompt = f"Rédige un court e-mail de prospection très persuasif pour vendre un site internet à l'entreprise '{lead['nom']}'."
    ...
    with tab3:
        st.header("🌐 Vos Serveurs d'Hébergement")
        if not liste_shops: st.info("Aucun site actif.")
        else:
            choix = st.selectbox("Sélectionnez le site :", [s for s in liste_shops])
            for s in liste_shops:
                if s == choix:
                    st.markdown(f"<div style='border: 2px dashed {s[3]}; padding: 20px; border-radius: 8px;'><h3>🏬 {s[0].upper()}</h3><p>{s[2]}</p></div>", unsafe_allow_html=True)
    ...
    with tab7:
        ...
        if not liste_shops: st.info("Aucun site à traduire.")
            ...
            shop_cible = st.selectbox("Site à traduire :", [s for s in liste_shops])
            langue = st.selectbox("Langue :", ["Anglais 🇺🇸", "Espagnol 🇪🇸"])
            if st.button("⚡ Traduire"):
                prompt = f"Traduis ce texte de boutique en {langue} : {[s[2] for s in liste_shops if s==shop_cible]}"
                nouveau_texte = outils.appeler_groq(prompt, temperature=0.3)
                outils.mettre_a_jour_boutique(shop_cible, nouveau_texte)
                st.success("🎉 Traduction injectée !")
